...
// crashWidenMaxPct caps a widened rung. ladder.GetInitialBidByDepth returns
// 0 for a percentage at or above 100, and percentage+tolerance must stay
// well under that; 45% per rung already covers a 10.10.2025-style flush
// within a few depths.
...
// where rowIdx is the row ladder.SettingsIndexOrBase actually serves — so a
// depth whose own row exists keeps its configured percentage, and every depth
// that falls back to the base row doubles per depth (2% → 4% → 8% → 16% →
// 32%), capped at crashWidenMaxPct. The bool reports whether widening changed
// the value.
func WidenedPercentage(settings []aggragates.StrategySettings, depthIndex int, crashActive bool) (float64, bool) {
	if len(settings) == 0 {
		return 0, false
	}
	rowIdx := ladder.SettingsIndexOrBase(settings, depthIndex)
...
func WidenSettingsForCrash(settings []aggragates.StrategySettings, depthIndex int, crashActive bool) []aggragates.StrategySettings {
	...
	widenedSettings[ladder.SettingsIndexOrBase(settings, depthIndex)].Percentage = widened
