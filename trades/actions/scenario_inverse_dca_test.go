...
func TestInverseDCA_GetProfitReturnsBaseCurrencyDelta(t *testing.T) {
	...
	gotProfit := profit.GetProfit(trade)
	// buyTotal (base) = 0.001
	...
	const want = -0.002
	if math.Abs(gotProfit-want) > 1e-9 {
		t.Errorf("inverse GetProfit = %v, want %v", gotProfit, want)
	}
}
...
func TestInverseDCA_GetQuantitiesReturnsRemainingInBase(t *testing.T) {
	...
	event := events.Events{Trade: trade}
	qty, side := quantities.GetQuantities(event)
	if side != "buy" {
